...
async function state (config) {
  ...
  _.each(config.semesters, (semesterConfig, semester) => {
    const start = moment.tz(new Date(semesterConfig.start), config.timezone)
    const end = moment.tz(new Date(semesterConfig.end), config.timezone)
    if (!(moment().isBetween(moment(start).subtract(config.people.startLoggingDaysBefore, 'days'), moment(end).add(config.people.endLoggingDaysAfter, 'days')))) {
      return
    }
    ...
    bulkState.find({ _id: semester }).upsert().replaceOne({
      sections, start: start.toDate(), end: end.toDate(),
      state: config.state,
      isSemester: true
    })
  })
