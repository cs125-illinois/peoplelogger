...
async function update (config) {
  ...
  let existingDiscourseUsers = _.keyBy(await discourseUsersCollection.find({}).toArray(), 'email')
  ...
  for (let page = 0; ; page++) {
    ...
    for (let user of newUsers) {
      ...
      let detailedInfo = cleanDetailedInfo(await callDiscourseAPI(config, {
        verb: 'get',
