...
async function discourse (config) {
  ...
  _.each(config.semesters, (semesterConfig, semester) => {
    ...
    const untrained = _(semesterPeople).values().filter(person => {
      return person.role === 'assistant' && person.assigned && !person.trained
    }).map('email').value()
    ...
    if (semester === currentSemester) {
      ...
      autoGroups.push(`${ semester }-Untrained`)
    }
    ...
    _.each(captains, email => {
      ...
      if (semester === currentSemester) {
        ...
        userGroups[email].push(`staff`)
        userGroups[email].push(`${semester}-Help`)
        userGroups[email].push(`${semester}-Untrained`)
      }
    })
    _.each(heads, email => {
      userGroups[email].push(`${semester}`)
