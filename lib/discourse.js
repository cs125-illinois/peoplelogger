...
async function discourse (config) {
  ...
  _.each(config.semesters, (semesterConfig, semester) => {
    ...
    expect(students.length + captains.length + TAs.length + CAs.length + developers.length + dataanalysts.length + inactive.length).to.equal(semesterPeople.length)
    ...
    if (semester === currentSemester && config.populateHelpGroup) {
