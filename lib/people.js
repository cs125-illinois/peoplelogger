...
async function addTASections (config, people, currentSemester, sheetID) {
  ...
  for (let person of persons) {
    ...
    people[email].sections = person['Sections'].split(',')
    expect(people[email].sections).to.be.ok()
    console.log(email, people[email].sections)
  }
  return people
}
...
async function staff (config) {
  ...
  for (let currentSemester of currentSemesters) {
    ...
    await addTASections(config, currentStaff, currentSemester, config.semesters[currentSemester].sheets.staff)
...
module.exports = exports = {
  staff, students, getSemesterPeople, getAllSemesterPeople, getAllPeople
}
