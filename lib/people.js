...
async function getFromAD (config, IDs) {
  const client = new LDAP({ url: 'ldap://ad.uillinois.edu/' })
  const people = {}
  return await client.starttls({}, []).then(async () => {
    const login = await client.bind(config.secrets.AD.username, config.secrets.AD.password)
    for (const ID of IDs) {
...
async function staff (config) {
  ...
  for (let currentSemester of currentSemesters) {
    ...
    let currentStaff = await getFromAD(config, staff.all)
