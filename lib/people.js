...
async function getNetIDsFromSheet (config, sheetID, worksheet, key = 'NetID') {
  const sheets = await googleSpreadsheetToJSON({
    spreadsheetId: sheetID,
    credentials: config.secrets.google,
    propertyMode: 'none',
  ...
  for (let index in sheets) {
    ...
    const name = worksheet[index]
    for (let person of sheet) {
      const netID = person[key]
      expect(netID).to.be.ok()
      expect(netID.endsWith('@illinois.edu')).to.not.be.true()
  ...
  for (let person of _.values(distinctPeople)) {
    ...
    try {
      ...
      if (err.response.statusCode === 404) {
        ...
        config.log.warn(`${err.response.statusCode} fetching Gravatar for ${person.email}`)
      }
    }
...
async function staff (config) {
  ...
  for (let currentSemester of currentSemesters) {
    ...
    const trainedSheet = config.semesters[currentSemester].sheets.trainedStaff
    const trainedStaff = trainedSheet ? await getNetIDsFromSheet(config, trainedSheet, ['CAs']) : undefined
    ...
    for (const email of _.keys(trainedStaff)) {
      if (!(email in currentStaff)) {
        console.warn(`${email} from trained sheet was probably typed incorrectly...`)
      }
    }


    for (const key of _.keys(currentStaff)) {
      const staffMember = currentStaff[key]
      ...
      if (staffMember.role === 'assistant') {
        if (trainedSheet) {
          staffMember.trained = staffMember.email in trainedStaff
        }
        if (moment(config.runTime).isBefore(moment.tz(new Date(semesterConfig.start), config.timezone).add(config.people.CAassignedGracePeriodDays, 'days'))) {
          assigned = true
        } else {
          if (staffMember.labs && staffMember.labs.length > 0) {
            assigned = true
        ...
        if (!staffMember.trained) {
          if (config.semesters[currentSemester].considerPreviousAsTrained) {
            const previousExperience = await peopleCollection.find({
              active: true,
              staff: true,
              email: staffMember.email,
              semester: { $ne: currentSemester }
            }).toArray()
            staffMember.trained = previousExperience.length > 0
          }
        }
        if (assigned && !staffMember.trained) {
          // console.trace(`${ staffMember.email } is assigned but not trained`)
        }
        active = assigned && staffMember.trained
      } else {
        assigned = true
        active = true
      }
      staffMember.assigned = assigned
      staffMember.active = active
    }

    currentStaff = _.pickBy(currentStaff, ({ active }) => active)
...
async function students (config) {
  ...
  for (let currentSemester of currentSemesters) {
    ...
    const enrolledNetIDs = _.clone(currentNetIDs)
    ...
    _.each(currentStudents, student => {
      ...
      student.temporary = enrolledNetIDs.indexOf(student.netID) === -1
