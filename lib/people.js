...
async function addSurvey (config, currentSemester) {
  if (!currentSemester) {
  ...
  const sheets = await googleSpreadsheetToJSON({
    spreadsheetId: config.semesters[currentSemester].sheets.survey,
    credentials: config.secrets.google,
    propertyMode: 'none',
  ...
  for (let response of sheets) {
    let result = {}
    _.each(config.survey, (info, name) => {
      ...
      if (info.type === 'free') {
        ...
        if (Array.isArray(info.options)) {
          ...
          chosenOptions = _.pickBy(info.options, value => {
            return allOptions.indexOf(value) !== -1
          })
        }
        let missedOptions = _.difference(allOptions, _.values(chosenOptions))
        if (missedOptions.length > 1) {
          config.log.debug(`Problem answer: ${ missedOptions.join(',') }`)
  ...
  _.each(config.survey, (info, name) => {
    ...
    if (info.options) {
      ...
      _.each(seen[name].missed, (count, option) => {
        if (count > config.surveyMissedThreshold) {
...
async function addAllowedStaff (config, currentStaff, currentSemester) {
  ...
  _.each(currentStaff, staffMember => {
    ...
    bulkPhotos.find({
      ...
      $set: {
        allowed: {
          [`${ currentSemester }`]: staffMember.allowed
        }
      }
    })
  })
