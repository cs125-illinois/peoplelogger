...
async function staff (config) {
  ...
  for (let currentSemester of currentSemesters) {
    ...
    let dataAnalysts = []
    try {
      dataAnalysts = _(TAsAndCDs).pickBy(results => {
        return 'Data' in results && results['Data'].trim() !== ''
      }).keys().value()
    } catch (err) { }
    ...
    _.each(currentStaff, staffMember => {
      ...
      if (dataAnalysts.indexOf(staffMember.email) !== -1) {
        staffMember.dataanalyst = true
      }
    ...
    _.each(staff.assistants, email => {
      ...
      if (person.labs && person.labs.length > 0) {
        ...
        active = true
      } else if (person.sawOfficeHours && moment(person.sawOfficeHours).isAfter(moment(config.runTime).subtract(config.people.officeHourGracePeriodDays, 'days'))) {
        active = true
      }
...
async function students (config) {
  ...
  for (let currentSemester of currentSemesters) {
    ...
    if (semesterConfig.sheets.provisional &&
