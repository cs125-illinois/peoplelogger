...
async function mailman (config) {
  ...
  syncList(
    ...
    `ustaff`,
    allPeople,
    ({ role }) => { return role === 'head' || role === 'associate' || role == 'captain' },
    null,
    dryRun
  )
  syncList(
  ...
  syncList(
    ...
    `data`,
    allPeople,
    person => { return person.role === 'developer' && person.dataanalyst },
    null,
    dryRun
  )
  syncList(
  ...
  */
  syncList(
    ...
    `captains`,
    allPeople,
    ({ role, active }) => { return role === 'captain' },
    null,
    dryRun
  )
  syncList(
    ...
    `cas`,
    allPeople,
    ({ role, active }) => { return (role === 'assistant' || role === 'associate') && active },
    true,
    dryRun
  )
  syncList(
  ...
  syncList(
    ...
    allPeople,
    ({ role, active }) => { return role === 'assistant' },
    true,
    dryRun
  )
  syncList(
    ...
    `untrained`,
    allPeople,
    ({ role, trained, assigned }) => { return role === 'assistant' && trained === false && assigned === true },
    true,
    dryRun
  )
  syncList(
  ...
  syncList(
    ...
    `blueteam`,
    allPeople,
    ({ active, team }) => { return active && team === 'blue' },
    ({ role, active }) => { return role === 'TA' && active },
    dryRun
  )
  syncList(
    ...
    `orangeteam`,
    allPeople,
    ({ active, team }) => { return active && team === 'orange' },
    ({ role, active }) => { return role === 'TA' && active },
    dryRun
  )
  syncList(
